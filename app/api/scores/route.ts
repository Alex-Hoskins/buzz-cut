...
// Minimum passes required to physically clear all hair on each level.
// Derived from Math.ceil(hairBoundsWidth / clipperDiameter) where clipperDiameter = 44px.
...
export async function GET(req: NextRequest) {
  ...
  // ZRANGE returns alternating [member, score, member, score, ...] with WITHSCORES
  const raw = await redis.zrange(levelKey(levelId), 0, 9, {
    withScores: true,
  });
  ...
  for (let i = 0; i < raw.length; i += 2) {
    ...
    scores.push(typeof raw[i + 1] === "string" ? parseFloat(raw[i + 1] as string) : (raw[i + 1] as number));
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!HANDLE_RE.test(handle)) {
    return NextResponse.json(
      {
        error:
          "Handle must be 3–12 characters: letters, numbers, underscores, or hyphens",
      },
      { status: 400 }
    );
  }
  ...
  // Always update the handle so handle changes propagate even without a score improvement.
